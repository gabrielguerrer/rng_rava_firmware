...
// RAVA : Rename USB
#define USB_PRODUCT RAVA_USB_PRODUCT
#define USB_MANUFACTURER RAVA_USB_MANUFACTURER
#define USB_VID RAVA_USB_VID
#define USB_PID RAVA_USB_PID

// RAVA : Disable receive and transmit led lights
#undef TXLED0
#undef TXLED1
#undef RXLED0
#undef RXLED1
...
static bool USB_SendSNDescriptor() {
	...
	for(uint8_t i = 0; i < 8; i++) {
   		if (!(SendControl(sn[i]) && SendControl(0))) {
        	return false;
        }
	}
    return true;
