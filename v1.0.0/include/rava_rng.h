...
class RNG
{
  public:
    ...
    uint8_t gen_int8(uint8_t int_max);
    void send_int8s(uint32_t n_ints, uint8_t int_max);
    uint16_t gen_int16(uint16_t int_max);
    void send_int16s(uint32_t n_ints, uint16_t int_max);

    void start_bytes_stream(uint16_t n_bytes, uint8_t postproc_id, uint16_t delay_ms);
